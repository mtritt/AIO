...
def plot_bbox(easy_ocr_result, image):
    boxes = [line[0] for line in easy_ocr_result] # tọa độ 4 góc của khung
    texts = [line[1] for line in easy_ocr_result] # đoạn text đọc được 
    scores = [line[2] for line in easy_ocr_result] # độ tin cậy
...
def create_reader(languages: list):
    return easyocr.Reader(languages, gpu=False) # mô hình chạy bằng cpu 
